...
RSpec.describe ROM::SQL::Function, :postgres do
  ...
  describe '#method_missing' do
    ...
    it 'raises error when is set already' do
      expect {
        func.count(:id).upper.sql_literal(ds)
  ...
  describe '#case' do
    context 'when condition argument is a Hash' do
      it 'returns an sql expression' do
        expect(
          func.case('1' => 'first', else: 'last').sql_literal(ds)
    ...
    context 'when the hash consists of expressions' do
      it 'returns an sql expression' do
        expect(
          func.case(users[:id].is([1, 2]) => 'first', else: 'last').sql_literal(ds)
  ...
  describe '#over' do
    ...
    example 'with the PARTITION BY clause which is aliased' do
      expect(
        func.row_number.over(partition: :name, order: :name).as(:row_numbers).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (PARTITION BY "name" ORDER BY "name") AS "row_numbers"'
      )
    end

    example 'with the frame clause' do
      expect(func.row_number.over(frame: :all).sql_literal(ds)).to eql(
        'ROW_NUMBER() OVER (ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)'
      )

      expect(func.row_number.over(frame: :rows).sql_literal(ds)).to eql(
        'ROW_NUMBER() OVER (ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)'
      )

      expect(
        func.row_number.over(frame: { range: :current }).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (RANGE BETWEEN CURRENT ROW AND CURRENT ROW)'
      )

      expect(
        func.row_number.over(frame: { range: [:current, :end] }).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (RANGE BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING)'
      )

      expect(
        func.row_number.over(frame: { range: [:start, :current] }).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)'
      )

      expect(
        func.row_number.over(frame: { range: [-3, 3] }).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (RANGE BETWEEN 3 PRECEDING AND 3 FOLLOWING)'
      )

      expect(
        func.row_number.over(frame: { rows: [-3, :current] }).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (ROWS BETWEEN 3 PRECEDING AND CURRENT ROW)'
      )

      expect(
        func.row_number.over(frame: { rows: [-3, :end] }).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (ROWS BETWEEN 3 PRECEDING AND UNBOUNDED FOLLOWING)'
      )

      expect(
        func.row_number.over(frame: { rows: [3, 6] }).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (ROWS BETWEEN 3 FOLLOWING AND 6 FOLLOWING)'
      )

      expect(
        func.row_number.over(frame: { rows: [-6, -3] }).sql_literal(ds)
      ).to eql(
        'ROW_NUMBER() OVER (ROWS BETWEEN 6 PRECEDING AND 3 PRECEDING)'
      )
    end

    it 'supports aliases' do
      expect(
        func.row_number.over(order: :id).as(:row_no).sql_literal(ds)
  ...
  describe '#filter' do
    it 'adds basic FILTER clause' do
      expect(func.sum(:id).filter(:value).sql_literal(ds)).to eql(
        'SUM("id") FILTER (WHERE "value")'
      )
    end

    it 'supports restriction block' do
      expect(func.sum(:id).filter { id > 1 }.sql_literal(ds)).to eql(
        'SUM("id") FILTER (WHERE ("users"."id" > 1))'
      )
    end

    it 'supports combined conditions' do
      expect(func.sum(:id).filter(:value) { id > 1 }.sql_literal(ds)).to eql(
        'SUM("id") FILTER (WHERE (("users"."id" > 1) AND "value"))'
      )
    end

    it 'supports hashes' do
      expect(func.count(:id).filter(id: 1).sql_literal(ds)).to eql(
        'COUNT("id") FILTER (WHERE ("id" = 1))'
      )
    end
  end
  ...
  describe '#within_group' do
    it 'adds WITHIN GROUP clause' do
      expect(func.rank(:id).within_group(:value).sql_literal(ds)).to eql(
        'RANK("id") WITHIN GROUP (ORDER BY "value")'
      )
    end

    it 'works with a block' do
      expect(func.rank(:id).within_group { name }.sql_literal(ds)).to eql(
        'RANK("id") WITHIN GROUP (ORDER BY "users"."name")'
      )
    end
  end
